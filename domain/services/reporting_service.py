...
def portfolio_overview(proyecto_ids: Optional[List[int]] = None) -> Dict[str, Any]:
    rows = proyectos_repo.list_proyectos(estado=None, cliente=None, search=None)
    
    # Filtrar por proyectos permitidos si se especifica
    if proyecto_ids:
        rows = [r for r in rows if r["id"] in proyecto_ids]
...
def cost_table(proyecto_ids: Optional[List[int]] = None) -> List[Dict[str, Any]]:
...
def top_carga_personas(limit: int = 10, proyecto_ids: Optional[List[int]] = None) -> List[Dict[str, Any]]:
    """
    ...
    # Filtrar por proyectos permitidos si se especifica
    if proyecto_ids:
        active = [r for r in active if r["proyecto_id"] in proyecto_ids]
